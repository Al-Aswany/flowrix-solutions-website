...
import { contactInfo, demoHref, erpModules, navLinks, services } from "./data";
    ...
    <footer id="contact" className="bg-white px-5 pb-8 pt-20 sm:px-8 sm:pt-24 lg:px-12">
      <div className="mx-auto max-w-[1400px] border-t border-black/10 pt-10">
        <div className="grid gap-12 pb-16 lg:grid-cols-[1.4fr_.7fr_.7fr_.7fr]">
            ...
            <div className="mt-6 space-y-3">
              ...
              {contactInfo.whatsapp ? (
                <a
                  ...
                  className="flex items-center gap-2 text-sm font-medium text-[#25D366] transition hover:text-[#1aab52]"
                >
                  <WhatsAppIcon green />
                  {contactInfo.whatsapp}
                </a>
          ...
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#0a785f]">ERP Modules</p>
            <ul className="mt-5 space-y-3">
              {erpModules.map((mod) => (
                <li key={mod.title}>
                  <Link href="/#modules" className="text-sm text-[#5a6763] transition hover:text-[#0a1714]">
                    {mod.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#0a785f]">Services</p>
            <ul className="mt-5 space-y-3">
              {services.slice(0, 6).map((service) => (
                <li key={service.slug}>
                  <Link
                    href={`/services/${service.slug}`}
                    className="text-sm text-[#5a6763] transition hover:text-[#0a1714]"
                  >
                    {service.title}
                  </Link>
                </li>
          ...
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#0a785f]">Explore</p>
            <ul className="mt-5 space-y-3">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm text-[#5a6763] transition hover:text-[#0a1714]">
                    {link.label}
                  </Link>
                </li>
              ))}
...
function WhatsAppIcon({ green = false }: { green?: boolean }) {
  return (
    <span
      className={`inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${green ? "bg-[#25D366]" : "bg-[#b0bab6]"}`}
    >
      <svg aria-hidden="true" viewBox="0 0 24 24" className="h-3 w-3" fill="white">
